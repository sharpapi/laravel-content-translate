...
- Added a Laravel Boost skill (`resources/boost/skills/sharpapi-content-translate/SKILL.md`) so AI agents can discover the async submit/`fetchResults()` flow, the queued-job recipe, result shape and testing approach.
...
- Docs: fixed the README: the usage example claimed `translate()` returns the translated text; it returns a status URL for `fetchResults()`. Also closed an unterminated code block.
